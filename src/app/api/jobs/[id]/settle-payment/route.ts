...
async function rpc(method: string, params: unknown[]) {
  const response = await fetch(getBnbRpcUrl(), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ jsonrpc: "2.0", id: 1, method, params }), cache: "no-store" });
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const jobResult = await db.query("SELECT state, payment_state, metadata FROM jobs WHERE id=$1", [id]);
  if (!jobResult.rowCount) return NextResponse.json({ ok: false, error: "job_not_found" }, { status: 404 });
  const job = jobResult.rows[0] as { state: string; payment_state: string; metadata: Record<string, unknown> };
  ...
  try {
    ...
    const tx = txResponse.result as { to?: string; value?: string } | undefined; const receipt = receiptResponse.result as { status?: string; blockNumber?: string } | undefined;
    const valid = chainResponse.result === "0x61" && !!tx && !!receipt && receipt.status === "0x1" && typeof tx.to === "string" && tx.to.toLowerCase() === RECIPIENT.toLowerCase() && typeof tx.value === "string" && BigInt(tx.value) === BigInt(VALUE_WEI);
