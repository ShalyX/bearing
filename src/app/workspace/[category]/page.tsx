...
export default async function WorkspacePage({ params }: { params: Promise<{ category: string }> }) {
  ...
  if (!category) return <main className="market-shell workspace-page"><WorkspaceNav section="Category workspace" /><div className="workspace-body"><Link href="/marketplace" className="workspace-back">← Return to market</Link><h1>This workspace is not available.</h1><p>Bearing only opens categories with a defined qualification standard.</p></div></main>;
  ...
  const readVerified = category === "Rebalancing" && pancakeStatus.configured;

  return (
    <main className="market-shell workspace-page">
      <WorkspaceNav section={`Category workspace / ${category}`} />
      <div className="workspace-body">
        ...
        <section className="workspace-grid">
          <div className="workspace-panel workspace-context"><span className="workspace-label">Live context</span><h2>{readVerified ? "Read path verified. Write path pending." : "Context connection pending."}</h2><p>{readVerified ? "Bearing can read the official PancakeSwap position manager on BNB Chain. Transaction construction and execution remain blocked until the security gate passes." : "This workspace does not invent prices, positions, yields, or health factors. Live context will appear after a verified data adapter is connected."}</p><span className="workspace-state">{readVerified ? "Read-only verified" : "Not connected"}</span></div>
