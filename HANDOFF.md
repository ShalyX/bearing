...
The local and production `DATABASE_URL` are configured and the schema migration has been applied. The public `/api/ready` endpoint, production smoke suite, and production build pass. The local smoke command requires the local PostgreSQL service to be available; the current workstation returned 503 from local `/api/ready` because that service is unavailable. A real, read-only PancakeSwap position test persisted evidence job `3f2e93a6-608f-4d4d-9540-86eb862c9527`; no wallet action or onchain write was performed.
